...
smtp_server_endpoints = {'gmail': ('smtp.gmail.com', 587),
                         'wp': ('smtp.wp.pl', 465),
...
def get_imap_server_endpoint(email):
    ...
    return imap_server_endpoints[domain]

def get_smtp_server_endpoint(email):
    domain = email.split('@')[1].split('.')[0]
    return smtp_server_endpoints[domain]
...
class LoginWindow:
    ...
    def save_profile(self):
        profile = {'name': self.name_entry.get(),
                   'email': self.email_entry.get(),
                   'password': encryption.encrypt(self.password_entry.get())}
